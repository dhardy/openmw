...
void BSAFile::readHeader(const FileFinder& fileFinder)
{
    /*
    ...
    for (size_t i=0;i<filenum;i++)
    {
        ...
        try {
            fs.extPath =fileFinder.lookup(fs.name);
            external = StreamPtr(new FileStream(fs.extPath));
            ...
            // Leave fs.extPath empty
            assert(fs.extPath.empty());
        }
        
        lookup[fs.name] = i;
        
        //std::cout << "fs->name: " << fs.name << std::endl;
        //std::cout << "data_files.lookup: " << fs.extPath << std::endl;
        //std::cout << std::endl;
    }
...
void BSAFile::open(const string &file, const FileFinder& fileFinder)
{
    filename = file;
    input = StreamPtr(new FileStream(file));
    
    readHeader(fileFinder);
}

/** Open an archive from a generic stream. The 'name' parameter is
    used for error messages.
*/
void BSAFile::open(Mangle::Stream::StreamPtr inp, const std::string& name, const FileFinder& fileFinder)
{
    filename = name;
    input = inp;
...
StreamPtr BSAFile::getFile(const char *file)
{
    ...
    if (fs.extPath.empty())
        ...
        return StreamPtr(new SliceStream(StreamPtr(new FileStream(fs.extPath)),fs.offset, fs.fileSize));
    }
}
